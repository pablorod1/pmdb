...
const getVideos = async (movieId, type) => {
  try {
    const response = await fetch(
      `${BASE_URL}/${type}/${movieId}/videos?api_key=${API_KEY}&language=es-ES`
    );
    if (!response.ok) {
...
const getCredits = async (movieId, type) => {
  try {
    const response = await fetch(
      `${BASE_URL}/${type}/${movieId}/credits?api_key=${API_KEY}&language=es-ES`
    );
    if (!response.ok) {
...
const getProviders = async (movieId, type) => {
  try {
    const response = await fetch(
      `${BASE_URL}/${type}/${movieId}/watch/providers?api_key=${API_KEY}`
    );
    if (!response.ok) {
...
export const getTrendingMoviesWeek = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/trending/movie/week?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
...
export const getTrendingMoviesDay = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/trending/movie/day?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
...
export const getMoviesWithVideos = async () => {
  try {
    ...
    const moviesWithVideosPromises = nowPlayingMovies.movies.map(
      async (movie) => {
        try {
...
export const getTrendingSeriesWeek = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/trending/tv/week?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
...
export const getTrendingSeriesDay = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/trending/tv/day?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
...
export const getNowPlayingSeries = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/tv/on_the_air?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
...
export const getUpcomingSeries = async (page) => {
  try {
    const response = await fetch(
      `${BASE_URL}/tv/airing_today?api_key=${API_KEY}&language=es-ES&page=${page}`
    );
    if (!response.ok) {
    ...
    const data = await response.json();
    const currentDate = new Date().toISOString().split("T")[0]; // Fecha actual en formato YYYY-MM-DD
    const futureSeries = data.results.filter(
      (serie) => serie.first_air_date > currentDate
    ); // Filtra series futuras
    const totalPages = data.total_pages;
    return {
...
export const getSerieDetails = async (serieId) => {
  try {
    const response = await fetch(
      `${BASE_URL}/tv/${serieId}?api_key=${API_KEY}&language=es-ES`
    );
    if (!response.ok) {
...
export const getAllMoviesAndSeries = async () => {
  for (let i = 1; i <= 10; i++) {
    try {
      let popularMovies = await getPopularMovies(i);
      let popularSeries = await getPopularSeries(i);
      return { ...popularMovies.movies, ...popularSeries.series };
    } catch (error) {
